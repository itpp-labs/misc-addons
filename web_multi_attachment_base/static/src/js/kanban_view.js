...
odoo.define("kanban_view", function(require) {
    ...
    var FieldMultiFiles = AbstractField.extend({
        ...
        init: function() {
            ...
            if (!_.contains(this.supportedFieldTypes, this.field.type)) {
                var msg = _t(
                    "The type of the field '%s' must be a one2many or many2many field."
                );
    ...
    relational_fields.FieldOne2Many.include({
        _renderButtons: function() {
            ...
            this._super();
            var multy_attach =
                this.$buttons && this.$buttons.find(".o_button_select_files");
            var attachment_field =
                this.view.arch.tag === "kanban" &&
                this.view.arch.attrs.drop_attachments_field;
            if (
                !this.isReadonly &&
                attachment_field &&
                multy_attach &&
                ...
                var mf_widget = new FieldMultiFiles(
                    this.getParent(),
                    this.drop_att.name,
                    this.record,
                    {mode: "edit"}
