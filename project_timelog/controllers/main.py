...
class TimelogController(http.Controller):
    ...
    def init_timelog(self, **kwargs):
        ...
        time_subtasks = int(
            round(
                float(config.sudo().get_param("project_timelog.time_subtasks"))
                * convert_sec,
                0,
            )
        )
        time_warning_subtasks = time_subtasks - int(
            round(
                float(config.sudo().get_param("project_timelog.time_warning_subtasks"))
                * convert_sec,
                0,
            )
        )
        normal_time_day = int(
            round(
                float(config.sudo().get_param("project_timelog.normal_time_day"))
                * convert_sec,
                0,
            )
        )
        good_time_day = int(
            round(
                float(config.sudo().get_param("project_timelog.good_time_day"))
                * convert_sec,
                0,
            )
        )
        normal_time_week = int(
            round(
                float(config.sudo().get_param("project_timelog.normal_time_week"))
                * convert_sec,
                0,
            )
        )
        good_time_week = int(
            round(
                float(config.sudo().get_param("project_timelog.good_time_week"))
                * convert_sec,
                0,
            )
        )
