...
class TimelogController(http.Controller):
    ...
    def init_timelog(self, **kwargs):
        ...
        task_user_timelogs = timelog_obj.search(
            [("user_id", "=", user.id), ("work_id.task_id", "=", task.id)]
        )

        # get timelogs of other users in current task
        timelogs_other_users = timelog_obj.search(
            [("user_id", "!=", user.id), ("work_id.task_id", "=", task.id)]
        )
        ...
        if timelogs and timelogs[0].start_datetime is not False:
            date_object = datetime.datetime.strptime(
                timelogs[0].start_datetime, "%Y-%m-%d %H:%M:%S"
            )
        ...
        if timelogs and timelogs[-1].end_datetime is False:
            ...
            start_datetime = datetime.datetime.strptime(
                timelogs[-1].start_datetime, "%Y-%m-%d %H:%M:%S"
            )
        ...
        if task_user_timelogs:
            ...
            all_work = request.env["account.analytic.line"].search(
        ...
        day_timer = 0
        timelog_today = timelog_obj.search(
            [
                ("user_id", "=", user.id),
                (
                    "start_datetime",
        ...
        if first_weekday == "monday":
            first_day_of_week = today - datetime.timedelta(
                datetime.datetime.weekday(today)
            )
            last_day_of_week = today + datetime.timedelta(
                6 - datetime.datetime.weekday(today)
            )
        elif first_weekday == "sunday":
            first_day_of_week = today - datetime.timedelta(
                1 + datetime.datetime.weekday(today)
            )
            last_day_of_week = today + datetime.timedelta(
                5 - datetime.datetime.weekday(today)
            )

        timelog_this_week = timelog_obj.search(
            [
                ("user_id", "=", user.id),
                (
                    "start_datetime",
                    ">=",
                    first_day_of_week.strftime("%Y-%m-%d 00:00:00"),
                ),
                (
                    "start_datetime",
                    "<=",
                    last_day_of_week.strftime("%Y-%m-%d 23:59:59"),
                ),
            ]
        )
        ...
        if timelogs_other_users:
            ...
            for user_id in another_users:
                res = timelog_obj.search(
                    [("user_id", "=", user_id), ("work_id.task_id", "=", task.id)]
                )
                ...
                sum_another_timelog = datetime.timedelta(
                    seconds=round(sum_another_timelog, 0)
                )
                current_user = request.env["res.users"].browse(user_id)
                second_timer_info.append(
                    current_user.name + ": " + str(sum_another_timelog) + "\n"
                )
        ...
        time_subtasks = int(
            round(
                float(config.get_param("project_timelog.time_subtasks")) * convert_sec,
                0,
            )
        )
        time_warning_subtasks = time_subtasks - int(
            round(
                float(config.get_param("project_timelog.time_warning_subtasks"))
                * convert_sec,
                0,
            )
        )
        normal_time_day = int(
            round(
                float(config.get_param("project_timelog.normal_time_day"))
                * convert_sec,
                0,
            )
        )
        good_time_day = int(
            round(
                float(config.get_param("project_timelog.good_time_day")) * convert_sec,
                0,
            )
        )
        normal_time_week = int(
            round(
                float(config.get_param("project_timelog.normal_time_week"))
                * convert_sec,
                0,
            )
        )
        good_time_week = int(
            round(
                float(config.get_param("project_timelog.good_time_week")) * convert_sec,
                0,
            )
        )
