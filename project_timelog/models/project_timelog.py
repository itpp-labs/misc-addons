...
class ProjectTimelog(models.Model):
    ...
    work_id = fields.Many2one(
        "account.analytic.line", "Sub Task", required=True, index=True
    )
    ...
    end_datetime_active = fields.Datetime(
        string="Timer End date",
        help="End date. Equal to now if timer is not stopped yet.",
    ...
    corrected_duration_active = fields.Float(
        string="Timer Corrected duration", store="True"
    )
        ...
        time_subtask = int(
            round(
                float(
                    ...
                    .sudo()
                    .get_param("project_timelog.time_subtasks")
                )
                * 3600,
                0,
            )
...
class AccountAnalyticLine(models.Model):
    ...
    def play_timer(self):
        ...
        if first_timelog and first_timelog[0].end_datetime is not False:
            if (
                first_timelog[0].end_datetime is not False
                and first_timelog[0].end_datetime.day != current_date.day
            ):
                # there are timelogs yesterday
                return self.show_warning_message(
