odoo.define("project_timelog.timelog", function(require) {
    ...
    TimeLog.Manager = Widget.extend({
        ...
        on_notification_do: function(channel, message) {
            ...
            if (
                Array.isArray(current_channel) &&
                current_channel[1] === "project.timelog"
            ) {
                try {
    ...
    TimeLog.TimelogWidget = Widget.extend({
        init: function(parent) {
            ...
            if (window.Audio) {
                ...
                this.error = new Audio(
                    session.url(
                        "/project_timelog/static/src/audio/offline" + this.audio_format
                    )
                );
            }
            this.status = "stopped";
        ...
        change_audio: function(name) {
            if (window.Audio) {
                this.audio.src =
                    session.url(
                        "/project_timelog/static/src/audio/" + name + this.audio_format
                    ) || this.stop_src;
                this.audio.play();
            }
        },
        ...
        add_favicon: function() {
            if (this.status === "stopped") {
                $('link[type="image/x-icon"]').attr(
                    "href",
                    "/project_timelog/static/src/img/favicon_play.ico"
                );
            } else if (this.status === "running") {
                $('link[type="image/x-icon"]').attr(
                    "href",
                    "/project_timelog/static/src/img/favicon_stop.ico"
                );
            }
        },
        updateView: function() {
        ...
        timerTimeLimited: function() {
            ...
            model
                .call("stop_timer", [this.config.work_id, true, false])
                .then(function() {
                    self.finish_status = true;
        ...
        add_title: function(timer_name, task_name, description) {
            ...
            $("#clock0").attr(
                ...
                "Subtask: " +
                    timer_name +
                    "\n\nTotal time of the subtask.\n\n* White: time is less than " +
                    tws[0] +
                    " hours " +
                    tws[1] +
                    " minutes;\n* Short Signal: time is " +
                    tws[0] +
                    " hours " +
                    tws[1] +
                    " minutes;\n* Yellow: time between " +
            ...
            $("#clock1").attr(
                ...
                "Task: " +
                    task_name +
                    "\n\nTotal time for the task (includes logs of other users): " +
                    description +
