...
class Import(models.TransientModel):
    ...
    def get_map(self, options):
        if options.get("settings"):
            settings = self.env["base_import_map.map"].search(
                [("id", "=", options.get("settings"))]
            )
            return settings
    ...
    def parse_preview(self, options, count=10):
        ...
        if settings:
            res["matches"] = {
                r.column_number: [str(r.field_name)] for r in settings.line_ids
            }
        return res
    ...
    def do(self, fields, options, dryrun=False):
        ...
        if not dryrun and options["save_settings"]:
            model_id = (
                self.env["ir.model"].search([("model", "=", str(self.res_model))]).id
            )
...
class SettingImport(models.Model):
    ...
    file_read_hook = fields.Text(
        string="File read hook",
        help="""
Update values of the file. Use variable ``row`` to update values in a row.""",
    ...
    line_ids = fields.One2many(
        "base_import_map.line", "setting_id", string="Settings line"
    )
