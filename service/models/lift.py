...
class LiftSetup(models.Model):

    _name = 'drm.lift'
    _description = 'Work Center on lift'
    ...
    resource_id = fields.Many2one('resource.resource',
                                  ...
                                  ondelete='cascade',
                                  required=True)
    # routing_line_ids = fields.One2many('drm.routing.workcenter', 'workcenter_id', "Routing Lines")
    order_ids = fields.One2many('drm.workorders',
                                'workcenter_id',
                                "Repair Orders")
    ...
    workorder_ready_count = fields.Integer('# Read Work Orders',
                                           compute='_compute_workorder_count')
    workorder_progress_count = fields.Integer('Total Running Orders',
                                              compute='_compute_workorder_count')
    workorder_pending_count = fields.Integer('Total Running Orders',
                                             compute='_compute_workorder_count')
    workorder_late_count = fields.Integer('Total Late Orders',
                                          compute='_compute_workorder_count')
    time_ids = fields.One2many('drm.workcenter.productivity',
                               'workcenter_id',
                               'Time Logs')
    ...
    blocked_time = fields.Float('Blocked Time',
                                ...
                                digits=(16, 2))
    productive_time = fields.Float('Productive Time',
                                   compute='_compute_productive_time',
                                   help='Productive hour(s) over the last month',
                                   digits=(16, 2))
    oee = fields.Float(compute='_compute_oee',
    ...
    def create(self, vals):
        vals['resource_id'] = self.resource_id.create({'name': vals['name'],
                                                       'resource_type': 'material'}).id
        res = super(LiftSetup, self).create(vals)
        return res

    @api.depends('order_ids.duration_expected', 'order_ids.workcenter_id', 'order_ids.state', 'order_ids.date_planned_start')
    def _compute_workorder_count(self):
        DrmWorkorder = self.env['drm.workorders']
        result = {wid: {} for wid in self.ids}
        result_duration_expected = {wid: 0 for wid in self.ids}
        # Count Late Workorder
        data = DrmWorkorder.read_group([('workcenter_id', 'in', self.ids), ('state', 'in', ('pending', 'ready')), (
        ...
        for res_group in res:
            ...
            if res_group['state'] in ('pending', 'ready', 'progress'):
                result_duration_expected[
                    res_group['workcenter_id'][0]] += res_group['duration_expected']
        for workcenter in self:
            workcenter.workorder_count = sum(count for state, count in result[
                                             workcenter.id].items() if state not in ('done', 'cancel'))
            workcenter.workorder_pending_count = result[
                workcenter.id].get('pending', 0)
            workcenter.workcenter_load = result_duration_expected[
                workcenter.id]
            workcenter.workorder_ready_count = result[
                workcenter.id].get('ready', 0)
            workcenter.workorder_progress_count = result[
                workcenter.id].get('progress', 0)
    ...
    def _compute_working_state(self):
        for workcenter in self:
            ...
            # to its `type_loss`, the workcenter is either being used or
            # blocked.
            time_log = self.env['drm.workcenter.productivity'].search([
            ...
            if not time_log:
                ...
                # the productivity line has a `loss_type` that means the
                # workcenter is being used
    ...
    def _compute_blocked_time(self):
        # TDE FIXME: productivity loss type should be only losses, probably
        # count other time logs differently ??
        data = self.env['drm.workcenter.productivity'].read_group([
    ...
    def _compute_performance(self):
        wo_data = self.env['drm.workorders'].read_group([
            ('date_start', '>=', fields.Datetime.to_string(
                datetime.datetime.now() - relativedelta.relativedelta(months=1))),
            ('workcenter_id', 'in', self.ids),
    ...
    def unblock(self):
        ...
        times = self.env['drm.workcenter.productivity'].search(
            [('workcenter_id', '=', self.id), ('date_end', '=', False)])
        times.write({'date_end': fields.Datetime.now()})
        return {'type': 'ir.actions.client', 'tag': 'reload'}


class DrmWorkcenterProductivityLoss(models.Model):
    _name = "drm.workcenter.productivity.loss"
...
class DrmWorkcenterProductivity(models.Model):
    _name = "drm.workcenter.productivity"
    ...
    workcenter_id = fields.Many2one('drm.lift',
                                    "Work Center",
                                    required=True)
    workorder_id = fields.Many2one('drm.workorders', 'Work Order')
    user_id = fields.Many2one(
        'res.users', "User",
        default=lambda self: self.env.uid)
    loss_id = fields.Many2one('drm.workcenter.productivity.loss',
                              "Loss Reason",
                              ondelete='restrict',
                              required=True)
    ...
    def button_block(self):
        self.ensure_one()
        self.workcenter_id.write({'working_state': 'blocked'})
