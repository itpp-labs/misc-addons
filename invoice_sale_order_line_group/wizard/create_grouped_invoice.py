    ...
    def make_grouped_invoice(self, cr, uid, ids, context=None):
        ...
        for line in sales_order_line_obj.browse(
            ...
            if (not line.invoiced) and (line.state not in ("draft", "cancel")):
                inv_line_id = sales_order_line_obj.invoice_line_create(
                    cr, uid, [line.id]
                )
                for lid in inv_line_id:
                    ...
                    invoices.setdefault(
                        ...
                        or line.order_id.partner_id.id,
                        [],
                    ).append((line.order_id, [lid]))
        ...
        for val in invoices.values():
            res = order_obj._make_invoice(
                cr,
                uid,
                val[0][0],
                reduce(lambda x, y: x + y, [l for o, l in val], []),
                context=context,
            )
            invoice_ref = ""
            origin_ref = ""
            for o in {o for o, l in val}:
...
class GroupedInvoiceWizard2(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        invoiced_lines = line_obj.browse(self._context.get("active_ids", [])).filtered(
            "invoiced"
        )
        draft_lines = line_obj.browse(self._context.get("active_ids", [])).filtered(
            lambda r: r.order_id.state in ["draft", "cancel"]
        )
