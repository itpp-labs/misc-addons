...
class ProductTemplate(models.Model):
    ...
    def _get_new_taxes(self, categ_id, old_category_taxes=None):
        ...
        new_taxes = {
            ...
            "supplier_taxes_id": [
                (4, new_tax) for new_tax in new_category.supplier_taxes_id.ids
            ],
        }
        if old_category_taxes:
            new_taxes["taxes_id"] += [
                (3, old_tax)
                for old_tax in old_category_taxes["taxes_ids"]
                if (4, old_tax) not in new_taxes["taxes_id"]
            ]
            new_taxes["supplier_taxes_id"] += [
                (3, old_tax)
                for old_tax in old_category_taxes["supplier_taxes_ids"]
                if (4, old_tax) not in new_taxes["supplier_taxes_id"]
            ]
        return new_taxes
...
class ProductCategory(models.Model):
    ...
    supplier_taxes_id = fields.Many2many(
        "account.tax",
        "product_category_supplier_taxes_id",
        "category_id",
        "tax_id",
        string="Vendor Taxes",
