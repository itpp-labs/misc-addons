...
class IrUiMenu(models.Model):
    ...
    _defaults = {
        "parent_id": lambda self, cr, uid, ctx: ctx
        and ctx.get("default_iframe_pages_group")
        and self.pool.get("ir.model.data").get_object_reference(
            cr, uid, "web_iframe_pages", "menu_top"
        )[1]
    }
...
class Page(models.Model):
    ...
    def get_default_menu_id(self):
        ...
        action = self.env["ir.actions.client"].create(
            {"name": name, "tag": "web_iframe.iframe"}
        )

        return self.env["ir.ui.menu"].create(
            {"name": name, "action": "ir.actions.client,%d" % action.id}
        )

    menu_id = fields.Many2one("ir.ui.menu", "Menu")
    menu_id_name = fields.Char("Entry")
    menu_id_parent_id = fields.Many2one(
        "ir.ui.menu", "Group", domain=[("iframe_pages_group", "=", True)], required=True
    )
