...
class CurrencyRateUpdateService(models.Model):
    ...
    max_delta_days = fields.Integer(
        "Max delta days",
        required=True,
        help="If the time delta between the rate date given by the webservice and the current date exeeds this value, then the currency rate is not updated in OpenERP.",
    )
...
class CurrencyRateUpdate(models.Model):
    ...
    cron = {
        ...
        "interval_number": 1,
        "interval_type": "weeks",
        "nextcall": time.strftime(
            "%Y-%m-%d %H:%M:%S", (datetime.today() + timedelta(days=1)).timetuple()
        ),  # tomorrow same time
        "numbercall": -1,
        "doall": True,
        ...
        for comp in companies:
            ...
            if main_curr_rec.rate != 1:
                raise orm.except_orm(
                    _("Error!"), ("Base currency rate should be 1.00!")
                )
            ...
            for service in comp.services_to_use:
                ...
                try:
                    ...
                    res, log_info = getter.get_updated_currency(
                        curr_to_fetch, main_curr, service.max_delta_days
                    )
...
class CurrenyGetterInterface(object):
    ...
    def check_rate_date(self, rate_date, max_delta_days):
        ...
        if days_delta > max_delta_days:
            raise Exception(
                "The rate timestamp (%s) is %d days away from today, which is over the limit (%d days). Rate not updated in OpenERP."
        ...
        if rate_date_str != datetime.strftime(datetime.today(), "%Y-%m-%d"):
            self.log_info = (
                "WARNING : the rate timestamp (%s) is not today's date" % rate_date_str
            )
            _logger.warning(
                "the rate timestamp (%s) is not today's date", rate_date_str
            )
...
class YahooGetter(CurrenyGetterInterface):
    ...
    def get_updated_currency(self, currency_array, main_currency, max_delta_days):
        ...
        return (
            ...
            self.log_info,
        )  # empty string added by polish changes
...
class AdminChGetter(CurrenyGetterInterface):
    ...
    def rate_retrieve(self, dom, ns, curr):
        ...
        res = {}
        xpath_rate_currency = (
            "/def:wechselkurse/def:devise[@code='%s']/def:kurs/text()" % (curr.lower())
        )
        xpath_rate_ref = (
    ...
    def get_updated_currency(self, currency_array, main_currency, max_delta_days):
        ...
        rate_date = dom.xpath(
            "/def:wechselkurse/def:datum/text()", namespaces=adminch_ns
        )[0]
        ...
        for curr in currency_array:
            ...
            _logger.debug(
                "Rate retrieved : 1 " + main_currency + " = " + str(rate) + " " + curr
            )
...
class ECBGetter(CurrenyGetterInterface):
    ...
    def rate_retrieve(self, dom, ns, curr):
        ...
        res = {}
        xpath_curr_rate = (
            "/gesmes:Envelope/def:Cube/def:Cube/def:Cube[@currency='%s']/@rate"
            % (curr.upper())
        )
    ...
    def get_updated_currency(self, currency_array, main_currency, max_delta_days):
        ...
        rate_date = dom.xpath(
            "/gesmes:Envelope/def:Cube/def:Cube/@time", namespaces=ecb_ns
        )[0]
        rate_date_datetime = datetime.strptime(rate_date, "%Y-%m-%d")
        self.check_rate_date(rate_date_datetime, max_delta_days)
        # we dynamically update supported currencies
...
# PL NBP # # ##########################################################################
class PLNBPGetter(
    CurrenyGetterInterface
):  # class added according to polish needs = based on class Admin_ch_getter
...
class BanxicoGetter(CurrenyGetterInterface):  # class added for Mexico rates
    ...
    def get_updated_currency(self, currency_array, main_currency, max_delta_days=1):
        ...
        for curr in currency_array:
            ...
            self.updated_currency[curr] = rate
            logger.debug(
                "Rate retrieved : " + main_currency + " = " + str(rate) + " " + curr
            )
    ...
    def get_updated_currency(self, currency_array, main_currency, max_delta_days):
        ...
        for curr in currency_array:
            ...
            if (dom.entries[0].cb_basecurrency == main_currency) and (
                ...
                rate_date_datetime = (
                    parser.parse(dom.entries[0].updated)
                    .astimezone(pytz.utc)
                    .replace(tzinfo=None)
                )
                self.check_rate_date(rate_date_datetime, max_delta_days)
                ...
                _logger.error(
                    ...
                    %s. Please check provider data format and/or source code."
                    % curr
                )
                raise UserError(
                    "Error !",
