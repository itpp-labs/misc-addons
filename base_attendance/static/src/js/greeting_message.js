    ...
    var GreetingMessage = Widget.extend(BarcodeHandlerMixin, {
        ...
        init: function(parent, action) {
            ...
            if (!action.attendance) {
                this.stop_listening();
                this.session
                    .user_has_group("base_attendance.group_hr_attendance_user")
                    .then(function(has_group) {
                        if (has_group) {
                            ...
                            self.next_action =
                                "base_attendance.hr_attendance_action_my_attendances";
                        }
                    });
                return;
            }
            ...
            if (
                this.next_action !==
                    "base_attendance.hr_attendance_action_kiosk_mode" &&
                this.next_action.tag !== "base_attendance_kiosk_mode"
            ) {
                this.stop_listening();
            }
            this.attendance = action.attendance;
            // Check in/out times displayed in the greeting message template.
            this.attendance.check_in_time = new Date(
                new Date(this.attendance.check_in).valueOf() -
            ...
            this.attendance.check_out_time =
                ...
                new Date(
                    new Date(this.attendance.check_out).valueOf() -
                        new Date().getTimezoneOffset() * 60 * 1000
                )
                    .toTimeString()
                    .slice(0, 8);
        ...
        welcome_message: function() {
            ...
            if (this.previous_attendance_change_date) {
                var last_check_out_date = new Date(
                    new Date(this.previous_attendance_change_date).valueOf() -
                        new Date().getTimezoneOffset() * 60 * 1000
                );
                if (
                    now.valueOf() - last_check_out_date.valueOf() >
                    1000 * 60 * 60 * 24 * 7
                ) {
        ...
        farewell_message: function() {
            ...
            if (this.previous_attendance_change_date) {
                var last_check_in_date = new Date(
                    new Date(this.previous_attendance_change_date).valueOf() -
                        new Date().getTimezoneOffset() * 60 * 1000
                );
                if (
                    now.valueOf() - last_check_in_date.valueOf() >
                    1000 * 60 * 60 * 12
                ) {
        ...
        on_barcode_scanned: function(barcode) {
            ...
            res_partner.call("attendance_scan", [barcode]).then(function(result) {
                if (result.action) {
