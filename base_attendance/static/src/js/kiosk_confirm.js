    ...
    var KioskConfirm = Widget.extend({
        events: {
            ...
            "click .o_hr_attendance_sign_in_out_icon": function() {
                ...
                hr_partner
                    .call("attendance_manual", [[this.partner_id], this.next_action])
                    .then(function(result) {
                        if (result.action) {
                            self.do_action(result.action);
                        } else if (result.warning) {
                            self.do_warn(result.warning);
                            self.$(".o_hr_attendance_sign_in_out_icon").removeAttr(
                                "disabled"
                            );
                        }
                    });
        ...
        start: function() {
            var self = this;
            self.session
                .user_has_group("base_attendance.group_hr_attendance_use_pin")
                .then(function(has_group) {
                    self.use_pin = has_group;
                    self.$el.html(
                        QWeb.render("BaseAttendanceKioskConfirm", {widget: self})
                    );
                    self.start_clock();
                });
