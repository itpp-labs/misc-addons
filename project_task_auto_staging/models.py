...
class ProjectTaskTypeAutoStaging(models.Model):
    ...
    def _check_delay_automove(self):
        if self.active_move and self.delay_automove <= 0:
            raise ValidationError("Value of 'Delay' field  must be greater than 0")
...
class ProjectTaskAutoStaging(models.Model):
    ...
    automove_to_stage_id = fields.Many2one(
        "project.task.type", readonly=True, related="stage_id.to_stage_automove_id"
    )
    ...
    _track = {
        "stage_id": {
            "project_task_auto_staging.mt_auto_move_task": lambda self, cr, uid, obj, ctx=None: ctx
            and ctx.get("auto_staging")
        }
    }
    ...
    def _get_allow_automove_one(self):
        self.ensure_one()
        self.allow_automove = (
            self.project_id.use_tasks
            and self.project_id.allow_automove
            and self.stage_id.active_move
            and self.stage_id.to_stage_automove_id
        )
    ...
    def _get_when_date_automove(self):
        ...
        self.when_date_automove = (
            datetime.datetime.strptime(self.write_date, DEFAULT_SERVER_DATETIME_FORMAT)
            + delta
        )
