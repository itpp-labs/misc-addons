...
class BackupController(http.Controller):
    ...
    def fetch_dashboard_data(self):
        ...
        usage_values = {
            datetime.strptime(
                r.date, DEFAULT_SERVER_DATE_FORMAT
            ).date(): r.total_used_remote_storage
            for r in values
        }
        ...
        for b_config in backup_configs:
            ...
            for backup in request.env["odoo_backup_sh.backup_info"].search(
                ...
                graph_values[
                    datetime.strptime(
                        backup.upload_datetime, DEFAULT_SERVER_DATETIME_FORMAT
                    ).date()
                ] += backup.backup_size
            b_config["graph"] = [
                {
                    ...
                    "values": [
                        {
                            "label": "Today"
                            if date == last_week_dates[-1]
                            else datetime.strftime(date, "%d %b"),
