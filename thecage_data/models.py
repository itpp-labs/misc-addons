...
class AccountAnalyticAccount(models.Model):
    ...
    remind_on_slots = fields.Integer(
        help="configure when to remind a customer about remaining slots",
        string="Remind on (slots)",
        default=2,
    )
    contract_slots = fields.Integer(
        string="Contract slots left",
        compute="_compute_contract_slots",
        readonly=True,
        help="remaining paid slots in contract",
        store=True,
    )
    ...
    def _compute_contract_slots(self):
        ...
        for l in self.invoice_line_ids:
            contract_slots += (
                not l.pitch_id.resource_id.has_slot_calendar
                and l.invoice_id.state == "paid"
                and l.invoice_id.type == "out_invoice"
                and l.quantity
                or not l.pitch_id.resource_id.has_slot_calendar
                and l.invoice_id.state == "paid"
                and l.invoice_id.type == "out_refund"
                and -l.quantity
                or l.pitch_id.resource_id.has_slot_calendar
                and l.invoice_id.state == "paid"
                and l.invoice_id.type == "out_invoice"
                and l.quantity / 2
                or l.pitch_id.resource_id.has_slot_calendar
        ...
        for l in self.order_line_ids:
            contract_slots += (
                not l.pitch_id.resource_id.has_slot_calendar
                and l.booking_state in ["consumed", "no_show"]
                and -l.product_uom_qty
                or l.pitch_id.resource_id.has_slot_calendar
                and l.booking_state in ["consumed", "no_show"]
                and -l.product_uom_qty / 2
            )
...
class SaleOrderTheCage(models.Model):
    ...
    def write(self, vals):
        ...
        for r in self:
            ...
            if vals.get("state") == "sent" and r.partner_id.confirmation_sms:
                ...
                for line in r.order_line.filtered(
                    lambda r: r.pitch_id
                ):  # filter not bookings
                    msg = (
                        "Successfully booked a pitch at The Cage %s!\n"
                        % line.venue_id.name
                    )
                    msg += "Pitch %s\n" % line.pitch_id.name
                    msg += "From: {} To {}\n".format(
    ...
    def remove_generated_lines(self):
        records = self.env["sale.order.line"].search(
            [("order_id", "=", self[0].id), ("automatic", "=", True)]
        )
...
class SaleOrderLine(models.Model):
    ...
    def send_booking_time_one(self):
        ...
        if self.booking_start and self.booking_end:
            ...
            composer = (
                self.env["mail.compose.message"].with_context(email_ctx).create({})
            )
            composer.send_mail()
    ...
    def _compute_line_active(self):
        for line in self:
            line.active = (
                line.booking_state != "cancelled" and line.order_id.state != "cancel"
            )
    ...
    def _cron_booking_reminder(self):
        lines72 = self.search(
            [
                ...
                (
                    "booking_start",
                    "<=",
                    (datetime.now() + timedelta(hours=72)).strftime(DTF),
                ),
            ]
        )
        lines48 = self.search(
            [
                ...
                (
                    "booking_start",
                    "<=",
                    (datetime.now() + timedelta(hours=48)).strftime(DTF),
                ),
            ]
        )
        ...
        for line in lines:
            if line.order_id.partner_id.reminder_sms:
                msg = (
                    "Your game at The Cage %s is coming up soon!\n" % line.venue_id.name
                )
                msg += "Pitch %s\n" % line.pitch_id.name
                msg += "From: {} To {}\n".format(
                    format_tz(line.booking_start, self.env.user.tz, DTF),
...
class ResPartnerReminderConfig(models.Model):
    ...
    confirmation_email = fields.Boolean(
        default=True, string="Booking email confirmation enabled"
    )
    confirmation_sms = fields.Boolean(
        default=True, string="Booking sms confirmation enabled"
    )
    reminder_sms = fields.Boolean(default=True, string="Booking sms reminder enabled")
    reminder_email = fields.Boolean(
        default=True, string="Booking email reminder enabled"
    )
...
class LinesWizard(models.TransientModel):
    ...
    def find_overlaps(self, pitch_id, booking_start, booking_end):
        overlaps = 0
        overlaps = self.env["sale.order.line"].search_count(
            [
                "&",
                "|",
                "&",
...
class GenerateBookingWizard(models.TransientModel):
    ...
    venue_id = fields.Many2one(
        "pitch_booking.venue", string="Venue", related="product_id.venue_id"
    )
    pitch_id = fields.Many2one("pitch_booking.pitch", string="Pitch")
    booking_start = fields.Datetime(string="Booking start")
    booking_end = fields.Datetime(string="Booking end")
    ...
    def default_get(self, cr, uid, fields, context=None):
        result = super(GenerateBookingWizard, self).default_get(
            cr, uid, fields, context=context
        )
        active_id = context and context.get("active_id", False)
        active_order = self.pool["sale.order"].browse(
    ...
    def generate_booking_lines(self):
        ...
        for line in range(0, self[0].quantity):
            ...
            overlap = bool(
                self.env["thecage_data.lines_wizard"].find_overlaps(
    ...
    def invoice_validate(self):
        for invoice_obj in self.filtered(lambda r: r.type == "out_refund"):
            for invoice_line_obj in invoice_obj.invoice_line:
                bookings = self.env["sale.order.line"].search(
                    [
                        ("pitch_id", "=", invoice_line_obj.pitch_id.id),
