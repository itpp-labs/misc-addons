odoo.define('ohada_reports.ohada_report', function (require) {
'use strict';
...
var accountReportsWidget = AbstractAction.extend(ControlPanelMixin, {
    ...
    init: function(parent, action) {
        ...
        if (this.report_model === undefined) {
            this.report_model = 'ohada.report';
        }
...
//        var text = $(e.target).siblings().val().replace(/[ \t]+/g, ' ');
    ...
    edit_footnote: function(e) {
        ...
        if (document.getElementById("edit_a_note")){
            document.getElementById("edit_a_note").style.display = "none";
        }
        if (document.getElementById("edit_a_note2")){
...
core.action_registry.add('ohada_report', accountReportsWidget);
